function simpAdd(expr0, expr1, reversed){
    ...
    if(expr0.type == "num" && expr1.type == "num"){
        return {type: "num", num: expr0.num.add(expr1.num)};
    }
    if(expr1.type == "num"){
        if(expr1.num == 0)
            return expr0;
        if(expr1.num < 0)
            return newSub(expr0, newNum(bigInt.zero.subtract(expr1.num)));
    }
    var hopeless = false;
    if(expr0.type == "binOp"){
        switch(expr0.op){
...
function simpMul(expr0, expr1){
    ...
    if(expr0.type == "num"){
        ...
        if(expr0.num == 1)
            return expr1;
        if(expr1.type == "num")
            return newNum(expr0.num.multiply(expr1.num));
    }
...
function simpDiv(expr0, expr1){
    ...
    if(expr0.type == "num"){
        ...
        if(expr0.num == 0)
            return newNum(0);
        if(expr1.type == "num"){
            var GCD = bigInt.gcd(expr0.num, expr1.num);
            if(GCD.eq(expr1.num))
                return newNum(expr0.num.divide(GCD));
            return newFrac(newNum(expr0.num.divide(GCD)), newNum(expr1.num.divide(GCD)));
        }
    }
...
function simpPow(expr0, expr1) {
    ...
    if (expr1.type == "num") {
        if (expr1.num.eq(0) && expr0.num.neq(0)) {
            return newNum(1);
        }
        if (expr1.num == "1") {
            return expr0;
        ...
        if (expr0.type == "num" && expr1.num.geq(0)) {
            return newNum(expr0.num.pow(expr1.num));
        }
    ...
    if (expr0.type == "num") {
        ...
        if (expr0.num.eq(1) || (expr0.num.eq(0) && expr1.num.neq(0)))
            return expr0;
    }
...
function simpProd(expr){
    ...
    if(!containsVar(expr2, "k")){
        var diff = floor(calc(simpAdd(expr1, exprNeg(expr0))) + 1);
        if(diff < 0)
            diff = bigInt(0);
